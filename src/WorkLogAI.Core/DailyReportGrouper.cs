...
/// <summary>
/// All the selected <see cref="ReportRow"/>s for a single calendar day, numbered
/// in arrival order for cross-referencing across the 項目/活動内容/結果 columns of
/// the exported report.
/// </summary>
...
{
    /// <summary>
    /// Groups <paramref name="rows"/> by <see cref="ReportRow.Date"/>, preserving
    /// the input's relative order both across days (the day of the first row seen
    /// for a given date determines its position) and within a day (stable order of
    ...
    public static IReadOnlyList<DailyReportRow> Group(IEnumerable<ReportRow> rows)
    {
        ...
        var result = new List<DailyReportRow>(dateOrder.Count);
